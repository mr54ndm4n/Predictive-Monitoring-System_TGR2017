function image_segmentation5()
close all
clear

%% Image Segmentation
rgb = imread('durian3.jpg');
...
BW_Ereskan = bwareaopen(BW_Ereskan, 100); %ลบสีขาว
BW = not(BW_Ereskan);
figure
% imshow(BW);
% title('bitwise image');

BW_grayPeach = rgb2gray(rgb);
BW_EdgePeach = edge(BW, 'sobel');
BW_ANDPEACH = BW & BW_EdgePeach;

se901 = strel('line', 2, 90); % expand size of line 90 degree
se01 = strel('line',2, 0);
BWsdilPeach = imdilate(BW, [se901 se01]); %ขยายสีขาว
imshow(BW_EdgePeach);
title('bitwise image');
...
imwrite(RGB2,'durianNumber.jpg');
command = ['python app.py ',int2str(length(B))];
% command
